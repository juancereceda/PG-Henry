...
const Slider = () => {
    ...
    async function GetProducts () {
        let ListaCombos = await axios.get('http://localhost:3001/products').then(response => response.data)
    ...
    function SlideAnimation () {
        if(SlideImages.current.children.length > 1){
            ...
            SlideImages.current.style.transform = `translateX(-${tamañoSlide + 50}px)`;
            const transition = () => {
    ...
    return(
        <StyledSlider ref={SlideImages}>
            {comboList.length > 0 ? comboList.map((combo) => {
                return (
                    <div key={combo._id}>
